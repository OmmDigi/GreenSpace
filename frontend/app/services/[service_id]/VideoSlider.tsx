    ...
    <div className={`relative w-full ${className}`}>
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-gray-50 to-gray-100 p-6">
        ...
        >
          {videos.map((video, index) => (
            <div
              ...
              style={{ width: `${100 / slidesToShow.desktop}%` }}
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-300 flex items-center justify-center">
                    <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <button
                        ...
                        className="bg-white/90 hover:bg-white rounded-full p-2 transition-all duration-200 hover:scale-110"
                      >
                      ...
                      >
                        {mutedVideos.has(video.id) ? (
                          <VolumeX size={18} className="text-gray-700" />
                        ) : (
                          <Volume2 size={18} className="text-gray-700" />
                        )}
                      </button>

                      <div className="bg-white/90 hover:bg-white rounded-full p-2 transition-all duration-200 hover:scale-110">
        ...
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white shadow-lg rounded-full p-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-110"
        >
        ...
        <button
          onClick={nextSlide}
          disabled={currentIndex >= videos.length - slidesToShow.desktop}
          className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white shadow-lg rounded-full p-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-110"
        >
      ...
      <div className="flex justify-center mt-4 space-x-2">
        {Array.from({ length: Math.ceil(videos.length / slidesToShow.desktop) }).map((_, index) => (
          <button
            key={index}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90 p-4">
          <div className="relative max-w-5xl max-h-full w-full">
            ...
            <div className="absolute bottom-4 left-4 bg-black/50 text-white px-3 py-2 rounded-lg">
              <p className="text-sm font-medium">{videos[modalVideoIndex].title}</p>
              <p className="text-xs opacity-75">{modalVideoIndex + 1} of {videos.length}</p>
            </div>
          </div>
        </div>
      )}
